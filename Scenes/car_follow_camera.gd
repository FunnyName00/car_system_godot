...
@export var joystick_sensibility := 3
...
func handle_joypad_rotation(delta: float) -> void:

	var joy_input = Input.get_vector("look_left", "look_right", "look_up", "look_down")
	
	if joy_input: # Deadzone check
		top_level = false
		# Rotate the parent (usually a spring arm or pivot)
		get_parent().rotate_y(-joy_input.x * joystick_sensibility * delta)
		top_level = true
